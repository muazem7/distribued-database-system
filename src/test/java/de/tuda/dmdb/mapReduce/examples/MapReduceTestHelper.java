...
public class MapReduceTestHelper {
  ...
  public static void compareResultSet(List<AbstractRecord> expectedList,
      List<HeapTable> resultTables) {
    for (HeapTable ht : resultTables) {
      ...
      while ((resultRec = scan.next()) != null) {
        ...
        Assertions.assertTrue(expectedList.remove(resultRec),
            resultRec + " should not be part of the result set.");
      }
      scan.close();
    }
    // After removing all records previously, there should not be any left.
    Assertions.assertTrue(expectedList.isEmpty(),
        "We expected more entries to be in the result list");
  }
}
