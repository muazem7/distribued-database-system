...
public class ShuffleSortTask extends ShuffleSortTaskBase {
  ...
  public ShuffleSortTask(HeapTable input, HeapTable output, int nodeId,
  ...
  public void run() {
    ...
    HashRepartitionExchange hashExchange =
        new HashRepartitionExchange(scanner, nodeId, nodeMap, port, partitionColumn);
    Sort sorter = new Sort(hashExchange, this.recordComparator);
    sorter.open();

    AbstractRecord rec;
    while ((rec = sorter.next()) != null)
      output.insert(rec);
