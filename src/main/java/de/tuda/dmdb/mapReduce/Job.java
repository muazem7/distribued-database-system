 ...
 * The set methods only work until the job is submitted
 *
 * <p>
 * Normally the user creates the application, describes various facets of the job via {@link Job}
...
public class Job {
  ...
  protected AbstractRecord outputPrototype; // used by the executor to init the ouput, must
   ...
   *
   * <p>
   * The <code>Job</code> modifies the <code>Configuration</code> so that any changes are visible to
   * the user.
   *
