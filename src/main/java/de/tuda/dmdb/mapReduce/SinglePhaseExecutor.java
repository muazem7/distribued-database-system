...
public class SinglePhaseExecutor extends MapReduceExecutor {
  ...
  public void submit(Job job) {
    ...
    for (int i = 0; i < this.numMappers; i++) {
      Future<?> future = cluster.submit(new SinglePhaseTask(job.getInputs().get(i),
          job.getOutputs().get(i), i, this.nodeMap, job.getConfiguration().getPartitionColumn(),
          job.getMapperClass(), job.getReducerClass()));
      tasks.add(future);
    }
