...
public abstract class MapReduceExecutor {
  protected int numMappers; // number of mappers used, usually same as number of input-partitions
                            // passed in by
  // the user
  protected int numReducers; // number of reducers used, can be ignored by different executor
                             // implementations
  protected ExecutorService cluster; // java thread pool used as cluster
  protected List<Future<?>> tasks; // each Future for a specific thread submitted to the pool, used
                                   // to track task/phase
  // completion
  ...
  protected void init(Job job) {
    ...
    if (0 == this.numReducers) { // second: if value was not set by user, we default to the same
                                 // number
      // as mappers
  ...
  protected void waitForCompletion() {
    ...
    Iterator<Future<?>> it = this.tasks.iterator(); // use iterator to be able to remove the task
                                                    // once it finished
    while (it.hasNext()) {
    ...
    System.out.println("MapReduceExecutor: Tasks completed! Took: "
        + TimeUnit.NANOSECONDS.toMicros(difference) + " microsecs");
  }
}
