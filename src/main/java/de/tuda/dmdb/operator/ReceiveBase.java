...
public abstract class ReceiveBase extends UnaryOperator {
  ...
  protected int numPeers; // indicates number of peers that have to finish processing before
                          // operator
  // finishes
  protected int listenerPort; // port on which receive server will be listening
  protected Queue<AbstractRecord> localCache; // cache passed to receive handler for storing
                                              // in-coming records
  protected TCPServer receiveServer; // used to store reference to receive server
  protected final AtomicInteger finishedPeers; // passed to receive server to track number of peers
                                               // that finish data
  // transfer
