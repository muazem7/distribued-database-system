...
public class TCPServer extends Thread {
  ...
  private final ServerSocket serverSocket; // Server socket to listen for incoming client
                                           // connections
  private final ExecutorService pool; // Thread pool to handle client connections concurrently
  private final Queue<AbstractRecord> localCache; // Reference to the local cache which will be used
                                                  // to the handler threads
  private final AtomicInteger finishedPeers; // Reference to the counter for the registration of
                                             // peers that finished sending
  // data
  private boolean running; // Flag to indicate when to stop the server (stop listening to incoming
                           // connections)
  ...
  public void stopServer() {
    try {
      this.running = false;
      System.out
          .println("TCPServer on " + serverSocket.getLocalSocketAddress() + " will be closed!");
      serverSocket.close();
    } catch (IOException e) {
