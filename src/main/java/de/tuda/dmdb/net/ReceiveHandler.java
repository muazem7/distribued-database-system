  ...
  public void handle(final Socket socket, ExecutorService pool) {
    pool.execute(() -> {
      ...
      try {
        ...
        System.err.println("Handler " + localSocketAddress + ": Unable to close connection to "
            + remoteSocketAddress);
        e.printStackTrace();
      }
      System.out.println(
          "Handler " + localSocketAddress + ": Connection to " + remoteSocketAddress + " closed");
  ...
  public void runTask(Socket socket) {
    ...
    try (ObjectInputStream in = new ObjectInputStream(socket.getInputStream());) {
      ...
      while ((input = (AbstractRecord) in.readObject()) != null) {
        ...
        if (!result) {
          System.err
              .println("Handler " + localSocketAddress + ": Could not add object to localCache!");
        } else {
      ...
      System.out.println("Handler " + localSocketAddress
