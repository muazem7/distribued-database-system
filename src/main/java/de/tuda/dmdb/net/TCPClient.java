...
public class TCPClient {
  ...
  public TCPClient(String host, int port, int sourceNodeId, int destinationNodeId)
    ...
    while (!connectionSuccess && retryCounter < maxRetries) {
      try {
        System.out.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
            + ": Connecting to " + host + " on " + port);
        this.socket = new Socket(host, port);
        connectionSuccess = true;
      } catch (ConnectException e) {
        System.err.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
            + ": The following error occured: " + e.getMessage());
        // wait and retry...
        ++retryCounter;
        System.out.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
            + ": Connection not successful, will retry to connect to " + host + " on port " + port);
        try {
          System.out.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
    ...
    if (!connectionSuccess || this.socket == null) {
      ...
      throw new RuntimeException("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
    ...
    try {
      ...
      throw new RuntimeException("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
          + ": Unable to connect! No connection established!", e);
    }
  }
  ...
  public boolean sendRecord(AbstractRecord record) {
    System.out.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
        + ": sending record " + record + " to server " + this.socket.getRemoteSocketAddress());
    try {
  ...
  public boolean close() {
    ...
    System.out.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
        + ": Closing connection to " + this.socket.getRemoteSocketAddress());
    try {
      this.socket.close();
    } catch (IOException e) {
      System.err.println("TCPClient-" + this.sourceNodeId + "-" + this.destinationNodeId
          + ": not close Socket to " + this.socket.getRemoteSocketAddress());
      e.printStackTrace();
      return false;
    }
    return true;
